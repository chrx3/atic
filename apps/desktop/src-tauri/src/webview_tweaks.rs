        ...
        unsafe {
            ...
            let handler = AcceleratorKeyPressedEventHandler::create(Box::new(
                move |_controller, args| {
                    ...
                    // consola: partir, menú de nueva consola/agente, cerrar. Ctrl+Shift+D
...
pub fn sync_controller_bounds(window: &WebviewWindow) -> Option<f64> {
    ...
    let user_scale = window
        ...
        .unwrap_or(1.0);
    // El estante no es el overlay: si usa `overlay_scale` (a menudo 1) con un
    // monitor al 150/200%, el toast cae a media pantalla y el ghost se despega
    // del mouse.
    let scale = if window.label() == "capture-shelf" {
        (f64::from(hwnd_dpi.max(96)) / 96.0)
            .max(tauri_scale)
            .max(0.01)
    } else {
