<script lang="ts">
  ...
  const { startDrag, endDrag } = createBubbleDrag(bubble, () => el, {
    ...
    onMove: ({ frame, cursor }) => {
      ...
      showRetachPreview("clipboard", retachReady(retachGesture, pill, frame, cursor));
    },
    onDrop: (info) => maybeRetachOnDrop(info.frame, info.cursor),
    onEnd: () => showRetachPreview("clipboard", false),
  });
  ...
  async function retachToPill(): Promise<void> {
    ...
    void emit("dock-tool-face", "clipboard").catch(() => cutRetach(false));
  }
  ...
  onMount(() => {
    ...
    return () => {
      ...
      window.clearTimeout(retachCutTimer);
      showRetachPreview("clipboard", false);
      endDrag();
      surfaces.resetInteraction();
...
{#if bubble.alive}
  ...
  >
    ...
    <header class="cf-head" onpointerdown={startDrag}>
      <i class="cf-grab" aria-hidden="true"></i>
      <!-- svelte-ignore a11y_no_static_element_interactions -->
