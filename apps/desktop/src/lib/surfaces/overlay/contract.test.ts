...
import { BUBBLE_MIN_H, BUBBLE_MIN_W, MARGIN, OVERLAY_LABEL } from "./contract";
...
function rustStrConst(source: string, name: string): string {
  const match = new RegExp(`const\\s+${name}\\s*:\\s*&str\\s*=\\s*"([^"]*)"`).exec(
    source,
  );
  if (!match) throw new Error(`No se encontró \`const ${name}\`. ¿Lo renombraron?`);
  return match[1];
...
describe("constantes gemelas con Rust", () => {
  ...
  it("el label de la ventana del overlay es el mismo", () => {
    expect(rustStrConst(overlayRs, "LABEL")).toBe(OVERLAY_LABEL);
  });
});
