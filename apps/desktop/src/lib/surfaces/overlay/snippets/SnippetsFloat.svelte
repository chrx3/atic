<script lang="ts">
  ...
  const { startDrag, endDrag } = createBubbleDrag(bubble, () => el, {
    ...
    onMove: ({ frame, cursor }) => {
      ...
      showRetachPreview("snippets", retachReady(retachGesture, pill, frame, cursor));
    },
  ...
  async function retachToPill(): Promise<void> {
    ...
    void emit("dock-tool-face", "snippets").catch(() => cutRetach(false));
  }
  ...
  onMount(() => {
    ...
    return () => {
      ...
      window.clearTimeout(retachCutTimer);
      showRetachPreview("snippets", false);
      endDrag();
      surfaces.resetInteraction();
