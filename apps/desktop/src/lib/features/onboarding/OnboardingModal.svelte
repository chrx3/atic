      ...
      {#key step}
        <div class="flex flex-col gap-4" in:tabPanel|local out:tabPanel|local>
          ...
          {:else if step === 2}
            ...
            <Field
              label={t("onboarding.transcribeLang")}
                ...
                >
                  <div class="flex min-w-0 flex-col gap-0.5">
                    <span class="text-sm font-medium text-text"
                      >{useLabel(model.id)}</span
                    >
                    <span class="truncate text-xs text-faint"
                      >{whisperModelLabel(model.id)}</span
                    >
                  </div>
                  {#if model.downloaded}
            ...
            {#if coreConflicts.length > 0}
              <Banner
                ...
                title={coreConflicts.length === 1
                  ...
                  : t("settings.shortcuts.conflictMany", {
                      count: coreConflicts.length,
                    })}
              >
            ...
            <ul class="flex list-none flex-col gap-3">
              {#each SETUP_SHORTCUTS as item (item.key)}
                <li class="flex items-start justify-between gap-3">
                  <div class="flex min-w-0 flex-col gap-0.5">
                    <span class="text-sm font-medium text-text"
                      >{t(`onboarding.setup.${item.id}.label`)}</span
                    >
                    <span class="text-xs text-faint"
                      >{t(`onboarding.setup.${item.id}.hint`)}</span
                    >
                  </div>
                  <HotkeyCapture
                    value={cfg[item.key]}
