<script lang="ts">
   ...
   * 24 que los iconos. Un círculo (ojo) y una asta recta.
   *
   * Portada de `apps/desktop/src/lib/AticMark.svelte`, con una diferencia:
   * en la web la mirada sigue solo a `pointermove` de la propia ventana (la
   * app además le pregunta al cursor real del escritorio). El resto —ojeras,
   * parpadeo, ladeo, rebote del ojo— es el mismo.
   *
   * `alive`: el círculo es una cara. Mira al puntero, parpadea y tiene ojeras.
   * `state`: qué herramienta corre AHORA; el glifo ocupa el lugar de los ojos.
   */

  import { prefersReducedMotion } from "./liquid";
  ...
  <g class="am-head">
    ...
    {#if facing}
      <g class="am-eye-l" transform="translate(10.12 11.15)">
        ...
        <path class="am-bag" d="M 0.836 1.241 Q 0.312 1.684 -0.062 1.861" />
      </g>
    ...
    {:else if markState === "recording"}
      <rect class="am-state am-rec" x="9.8" y="9.8" width="4.4" height="4.4" rx="1.15" />
